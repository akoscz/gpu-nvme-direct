...
\sysname{} operates on commodity hardware (AMD Ryzen 5800X, B550 motherboard)
...
On a WD SN530 (PCIe~3.0~$\times$4), \sysname{} achieves 2{,}666\MBs{} at
256\,KB block size with queue depth 32, reaching 78\% of the theoretical PCIe
link bandwidth. Compared to CPU-mediated baselines using a faster Samsung 980
PRO (PCIe~4.0~$\times$4), \sysname{} delivers up to 2.2$\times$ higher
throughput at 16\,KB block size due to its ability to maintain 32 in-flight
NVMe commands from a single GPU thread. We demonstrate the practical
applicability of this approach for LLM inference, where streaming 70B model
weights layer-by-layer from NVMe eliminates the CPU \texttt{memcpy} bottleneck
in the data loading pipeline.
...
\item \textbf{First demonstration of GPU-initiated NVMe I/O on consumer
  hardware.} We show that a GeForce RTX 3090 on an AMD B550 platform can
  autonomously issue NVMe commands, achieving 2{,}666\MBs{} (78\% of PCIe~3.0
  link bandwidth) without any CPU involvement in the data path.
...
\item \textbf{Characterization of PCIe P2P behavior on AMD consumer
  platforms.} We document the asymmetric P2P behavior of AMD B550/Vermeer:
...
\textbf{AMD Consumer (B550/X570, Matisse/Vermeer):} The AMD data fabric
...
the CPU from the data path, \sysname{} achieves 2.1\GBs{} on a PCIe~3.0
device, with projections of 4--6\GBs{} on PCIe~4.0 hardware, directly
...
\midrule
GPU & NVIDIA RTX 3090 (GA102, 24\,GB, PCIe 4.0 $\times$16) \\
CPU & AMD Ryzen 7 5800X (8C/16T, 3.8--4.7\,GHz) \\
Platform & AMD B550, DDR4-3200 (48\,GB) \\
NVMe (test) & WD SN530 1\,TB, PCIe 3.0 $\times$4 (VFIO) \\
...
\textbf{Note on device asymmetry.} The \sysname{} benchmarks use the WD SN530
(PCIe~3.0~$\times$4, $\sim$3{,}400\MBs{} theoretical max) because it is the VFIO
...
2{,}666/3{,}400 = \textbf{78.4\%} of its Gen3 link, while \texttt{cpu\_memcpy}
achieves 1{,}731/7{,}000 = \textbf{24.7\%}, \texttt{cpu\_pinned} reaches
1{,}409/7{,}000 = 20.1\%, and cuFile 1{,}396/7{,}000 = 19.9\%. The GPU-initiated
path thus extracts 3.2$\times$ more of the available link bandwidth than
...
achieves 2{,}666\MBs{} on the SN530, which is 78\% of the device's theoretical
PCIe~3.0~$\times$4 bandwidth ($\sim$3{,}400\MBs{}). The normalized comparison
above shows that CPU methods utilize only 20--25\% of their faster Gen4 link,
confirming that the GPU-initiated path is fundamentally more efficient at
extracting device bandwidth.
...
\begin{tabular}{lr}
\toprule
\textbf{Transfer Size} & \textbf{Throughput (MB/s)} \\
\midrule
4\,MB    & 2{,}122 \\
128\,MB  & 2{,}734 (peak) \\
669\,MB (1 Llama-70B Q6\_K layer) & 2{,}127 \\
\bottomrule
\end{tabular}
\end{table}

The peak throughput of 2{,}734\MBs{} at 128\,MB demonstrates that \sysname{}
can sustain near-optimal throughput for large sequential transfers. The slight
drop at 669\,MB is due to thermal throttling of the SN530 during sustained reads.

\subsection{Projected LLM Inference Performance}

To contextualize the I/O throughput results for LLM inference, we project
token-generation rates for Llama~2-70B in Q6\_K quantization ($\sim$42\,GB,
80~layers, $\sim$669\,MB/layer). These projections assume I/O-bound
layer-by-layer streaming where each token requires loading the full model
from storage---the regime where the model \emph{exceeds} GPU VRAM and
cannot be cached.

\begin{table}[t]
\centering
\caption{Projected Llama-70B inference performance (I/O-bound regime,
  model streamed from NVMe per token)}
\label{tab:llm-projection}
\small
\begin{tabular}{lrrrr}
\toprule
\textbf{Data Path} & \textbf{BW} & \textbf{1 Layer} & \textbf{80 Layers} & \textbf{tok/s} \\
\midrule
mmap+memcpy (current)         & 1.5\GBs{} & 446\,ms & 35.7\,s & 0.028 \\
\sysname{} (SN530 Gen3)       & 2.1\GBs{} & 319\,ms & 25.5\,s & 0.039 \\
\sysname{} (peak)             & 2.7\GBs{} & 248\,ms & 19.8\,s & 0.050 \\
\sysname{} (Gen4 $\times$4)   & 5.0\GBs{} & 134\,ms & 10.7\,s & 0.093 \\
Warm page cache + H2D         & 13\GBs{}  &  51\,ms &  4.1\,s & 0.244 \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Important context.} These token rates are inherently low because
the entire 42\,GB model must transit from storage per token. For reference,
llama.cpp achieves $\sim$1~tok/s for 70B models when weights reside in GPU
VRAM across two GPUs (tensor parallelism), and FlexGen~\cite{sheng2023flexgen}
achieves 1~tok/s for OPT-175B with aggressive CPU/disk offloading. The
contribution of Table~\ref{tab:llm-projection} is not the absolute token
rate---which is bounded by storage bandwidth---but the \textbf{relative
improvement} of the I/O data path: \sysname{} delivers 1.4--3.3$\times$ higher
throughput than CPU-mediated baselines for the same model and hardware.

For the scenario where Q8\_0 quantization ($\sim$70\,GB) \emph{exceeds}
our 48\,GB RAM, NVMe streaming via \sysname{} is the only viable path
without adding system memory. Combined with NanoQuant~\cite{chong2026nanoquant}
(sub-1-bit quantization compressing 70B to $\sim$8\,GB), the data volume
per token would drop from 42\,GB to $\sim$8\,GB, and \sysname{} at Gen4
bandwidth could reach $\sim$0.5~tok/s---approaching interactive use.
...
of this prior work has documented the specific asymmetry on AMD B550/Vermeer
...
memory, adding one PCIe hop compared to direct NVMe-to-GPU DMA. On our
platform, the GPU's PCIe~4.0~$\times$16 link to host memory provides
$\sim$25\GBs{} of bandwidth---7$\times$ more than the SN530's 3.4\GBs{}.
The NVMe device is therefore the bottleneck, not the extra hop, and Tier~1
loses no throughput from the indirection. However, with Gen5 $\times$4 NVMe
devices approaching 14\GBs{}, the host memory hop could become limiting for
multi-device configurations. This motivates \textbf{Tier~2}: enabling NVMe
DMA directly to GPU VRAM via the tinygrad P2P patches for the NVIDIA
open-source kernel module, which would eliminate the host memory intermediate
and enable scaling to multiple NVMe devices.
...
PCIe~3.0~$\times$4 link bandwidth at 256\,KB/QD=32. For comparison,
BaM~\cite{qureshi2023bam} reports 90\% of link bandwidth on enterprise A100
hardware with GPUDirect RDMA and queues in GPU HBM. The 12-point gap is
attributable to Tier~1's extra host memory hop: each I/O traverses the PCIe
fabric twice (NVMe $\rightarrow$ host DRAM $\rightarrow$ GPU), whereas BaM's
Tier~3 path traverses it once (NVMe $\rightarrow$ GPU HBM). Despite this
overhead, the normalized comparison (\S\ref{sec:eval}) shows that \sysname{}
extracts 3.2$\times$ more of the available link bandwidth than CPU-mediated
methods on the same platform.

\textbf{Synergy with quantization.} The I/O-bound token rates in
Table~\ref{tab:llm-projection} assume uncompressed (Q6\_K) model weights.
NanoQuant~\cite{chong2026nanoquant} demonstrates sub-1-bit quantization that
compresses Llama~2-70B to $\sim$8\,GB with acceptable perplexity degradation.
Combining \sysname{} with aggressive quantization would reduce the per-token
storage transfer from 42\,GB to $\sim$8\,GB, yielding a projected
$\sim$0.5~tok/s at Gen4 bandwidth---a 5$\times$ improvement over the
uncompressed \sysname{} path and approaching the regime where compute (not
I/O) becomes the bottleneck. This synergy is bidirectional: quantization
reduces I/O volume while GPU-initiated I/O reduces transfer latency, and
neither technique substitutes for the other.
...
\textbf{Toward end-to-end integration.} A key next step is integrating
\sysname{} into an LLM inference engine for end-to-end streaming layer
execution. The current evaluation demonstrates the I/O primitive; an
application-level benchmark would show the interaction between I/O pipelining
and GPU compute overlap. Specifically, while the GPU computes attention and
feed-forward operations on layer $n$, \sysname{} can prefetch layer $n+1$
from NVMe---a double-buffering strategy that hides I/O latency behind
computation when the compute time per layer exceeds the I/O time.

\textbf{Limitations.} (1)~Our evaluation compares devices of different
speeds (SN530 Gen3 vs.\ 980~PRO Gen4). While the bandwidth normalization
(\S\ref{sec:eval}) controls for this, a same-device comparison would
further strengthen the claims. (2)~Write support is implemented but not
...
AMD B550 platform, a single CUDA thread autonomously manages NVMe I/O
...
\sysname{} achieves 2{,}666\MBs{} (78\% of PCIe~3.0 link bandwidth),
outperforming CPU-mediated methods by up to 2.2$\times$ despite using a
slower NVMe device. The system demonstrates that GPU-initiated storage I/O
is not limited to enterprise hardware---consumer GPUs are physically capable
of this operation, and the main barriers are software (driver checks, P2P
topology) rather than hardware.

This work opens the door to GPU-centric storage access on commodity hardware,
with immediate applications in LLM inference, graph analytics, and any
